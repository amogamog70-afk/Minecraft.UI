...
local function playWorkspaceTrack(target, customTitle)
    ...
    if RadioTrackLabel then
        RadioTrackLabel.Text = "Playing: " .. tostring(customTitle)
    end
    if HUDPlayBtn then
...
HUDPlayBtn = Instance.new("TextButton")
...
HUDSoundInput.FocusLost:Connect(function(enterPressed)
    if HUDSoundInput.Text ~= "" then
        playWorkspaceTrack(HUDSoundInput.Text, "Custom Track")
    end
end)

RadioTrackLabel = Instance.new("TextLabel")
...
function Library:SetTheme(themeName)
    ...
    for _, block in ipairs(Library.Blocks) do
        ...
        if block.SubTabHolder then st(block.SubTabHolder, { BackgroundColor3 = t.Header }) end
        if block.SubTabStroke then st(block.SubTabStroke, { Color = t.StrokeActive }) end
...
function Library:CreateBlock(title, defaultPosition)
    ...
    function Block:AddSubTabs(tabList, callback, ...)
        ...
        SubTabHolder.BackgroundColor3 = Library.Theme.Header
        ...
        SubTabStroke.Color = Library.Theme.StrokeActive
        SubTabStroke.Transparency = 0.4
        ...
        for i, tabName in ipairs(tabList) do
            ...
            TabBtn.BackgroundColor3 = (i == 1) and Library.Theme.Card or Library.Theme.Header
            TabBtn.BorderSizePixel = 0
            ...
            TabBtn.MouseLeave:Connect(function()
                local isSel = (string.upper(tostring(strTabName)) == string.upper(tostring(Block.ActiveSubTab or "")))
                if not isSel then
                    smoothTween(TabBtn, DUR_FAST, { BackgroundColor3 = Library.Theme.Header, TextColor3 = Library.Theme.TextDim })
                end
            end)
    ...
    function Block:AddColorPixel(name, defaultColor, position, callback)
        ...
        -- HSV Color Picker Popup Window (Sleek Redesigned Compact UI)
        local PickerPopup = Instance.new("Frame")
        PickerPopup.Name = "ColorPicker_" .. name
        PickerPopup.Size = UDim2.new(0, 168, 0, 178)
        ...
        CloseBtn.ImageTransparency = 0.25
        CloseBtn.ZIndex = 604
        CloseBtn.Parent = PickerHeader

        CloseBtn.MouseEnter:Connect(function()
            smoothTween(CloseBtn, DUR_FAST, { ImageTransparency = 0 })
        end)
        CloseBtn.MouseLeave:Connect(function()
            smoothTween(CloseBtn, DUR_FAST, { ImageTransparency = 0.25 })
        end)
        ...
        SatValBox.Size = UDim2.new(0, 126, 0, 100)
        ...
        HueBar.Position = UDim2.new(0, 138, 0, 26)
        ...
        ValBar.Size = UDim2.new(0, 108, 0, 16)
        ValBar.Position = UDim2.new(0, 6, 0, 130)
        ...
        HexBoxBg.Size = UDim2.new(0, 44, 0, 16)
        HexBoxBg.Position = UDim2.new(0, 118, 0, 130)
        ...
        -- Quick Color Swatches Row (6 Preset Colors)
        local PresetColors = {
            Color3.fromRGB(255, 60, 60),   -- Red
            Color3.fromRGB(255, 160, 40),  -- Orange
            Color3.fromRGB(255, 220, 50),  -- Yellow
            Color3.fromRGB(60, 230, 110),  -- Green
            Color3.fromRGB(0, 210, 255),   -- Cyan
            Color3.fromRGB(180, 90, 255)   -- Purple
        }

        local SwatchContainer = Instance.new("Frame")
        SwatchContainer.Size = UDim2.new(1, -12, 0, 20)
        SwatchContainer.Position = UDim2.new(0, 6, 0, 151)
        SwatchContainer.BackgroundTransparency = 1
        SwatchContainer.ZIndex = 602
        SwatchContainer.Parent = PickerPopup
        ...
        for _, pColor in ipairs(PresetColors) do
            ...
            SwatchBtn.BackgroundColor3 = pColor
            SwatchBtn.BorderSizePixel = 0
            SwatchBtn.Text = ""
            SwatchBtn.ZIndex = 603
            SwatchBtn.Parent = SwatchContainer
            addCorner(SwatchBtn, 4)

            local SwatchStroke = Instance.new("UIStroke")
            SwatchStroke.Color = Library.Theme.Stroke
            SwatchStroke.Thickness = 0.8
